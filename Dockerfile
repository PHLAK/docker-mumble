...
RUN mkdir -pv /opt/mumble /etc/mumble/config
...
RUN apk add --update ca-certificates bzip2 tar wget && wget -qO- ${BZIP_URL} | tar -xjv --strip-components=1 -C /opt/mumble && apk del ca-certificates bzip2 tar wget && rm -rf /var/cache/apk/*
